...
class Dean extends CI_Controller
{
	...
	public function home() {
		if(isset($_SESSION['designation']) && $_SESSION['designation_fkid'] == 3)
		{
			...
			//$data['proplist']=$this->Proposal_AB->LoadProposals($this->session->department);
	...
	public function profile_settings() {
		if(isset($_SESSION['designation']) && $_SESSION['designation_fkid'] == 3)
		{
			...
			$this->load->view('Dean/dean_setting', $data);
		}else{
		    redirect(site_url());
		}
	}
	...
	{
		$this->load->model('Proposal_AB');
		$proplist=$this->Proposal_AB->LoadProposalsDean($this->session->office); 
		...
		if($this->input->post('save')) {
			...
			if($this->form_validation->run() === FALSE) {
				...
				if ($state === TRUE) {
					...
					redirect(site_url('Dean/profile_settings'), "refresh");

				} else {
				  	echo "<meta http-equiv='refresh' content='2; url=" . site_url('Dean/profile_settings') . "'>";
